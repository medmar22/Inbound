    ...
    class Inbound : RustPlugin
    {
        ...
        void OnEntitySpawned(BaseEntity entity)
        {
            ...
            if (entity is CargoPlane cargoPlane)
            {
                ...
                if (!SupplyPlayerCompatible() || AirdropPrecision?.Call<bool>("IsManagedDrop", cargoPlane) == true || FancyDrop?.Call<bool>("IsManagedDrop", cargoPlane) == true)
                    return;
        ...
        void OnSupplyDropLanded(SupplyDrop drop)
        {
            ...
            if (SupplyPlayerCompatible() || !HideSupplyAlert(calledDrop))
            {
                SendInboundMessage(Lang("SupplyDropLanded_", null, SupplyDropPlayer(calledDrop), Location(drop.transform.position)), configData.alerts.supplyDropLand);
            }
        }
